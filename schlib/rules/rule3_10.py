...
class Rule(KLCRule):
    ...
    def __init__(self, component):
        super(Rule, self).__init__(component, 'Rule 3.10', 'For components with a single default footprint, footprint field is filled with valid footprint filename')
    ...
    def check(self):
        ...
        if len(self.component.fields) >= 3:
            ...
            if len(fp_name) > 0:
                ...
                for ii in invalid:
                    if ii in fp_name:
                        self.verboseOut(Verbosity.HIGH, Severity.ERROR, fp_desc + " contains illegal character {c}".format(c=ii))
