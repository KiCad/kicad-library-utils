...
class Rule(KLCRule):
    ...
    def __init__(self, component):
        super(Rule, self).__init__(component, 'EC03 - Extra Checking', 'Check part reference and name position and alignment')
    ...
    def check(self):
        ...
        if len(self.component.filterPins(direction='D')) == 0:
            self.recommended_ref_pos = (0, top + 50)
            self.recommended_ref_alignment = 'C'

        # otherwise, the recommended is put it 50 mils before the first pin x position, right-aligned
        else:
            x = min([int(i['posx']) for i in self.component.filterPins(direction='D')]) - 100
            self.recommended_ref_pos = (x, top + 50)
        ...
        # if there is no pins in the bottom, the recommended position to name is at bottom-center, horizontally centered
        if len(self.component.filterPins(direction='U')) == 0:
            self.recommended_name_pos = (0, bottom - 50)
            self.recommended_name_alignment = 'C'

        # otherwise, the recommended is put it 50 mils after the last pin x position, left-aligned
        else:
            x = max([int(i['posx']) for i in self.component.filterPins(direction='U')]) + 50
            self.recommended_name_pos = (x, bottom - 50)
            self.recommended_name_alignment = 'L'
        ...
        return True if (ref_need_fix or name_need_fix) else False
