...
for libfile in args.libfiles:
    lib = SchLib(libfile)
    print_purple('library: %s' % libfile)
    for component in lib.components:
        ...
        if check3_1 or check3_2 or check3_6.count([]) < 3:
            print_green('component: %s' % component.name)

            if check3_1:
                print_yellow('\tViolations of rule 3.1')
                if args.verbose:
                    print_light_blue('\tUsing a 100mils grid, pin ends and origin must lie on grid nodes (IEC-60617).')

                for pin in check3_1:
                    print_white('\t\tpin: %s (%s), posx %s, posy %s, length: %s' %
                        (pin['name'], pin['num'], pin['posx'], pin['posy'], pin['length']))

            if check3_2:
                print_yellow('\tViolations of rule 3.2')
                if args.verbose:
                    print_light_blue('''\tFor black-box symbols, pins have a length of 100mils. Large pin numbers can be\n\taccomodated by incrementing the width in steps of 50mil.''')

                for pin in check3_2:
                    print_white('\t\tpin: %s (%s), dir: %s, length: %s' %
                        (pin['name'], pin['num'], pin['direction'], pin['length']))

            if check3_6.count([]) < 3:
                print_yellow('\tViolations of rule 3.6')
                if args.verbose:
                    print_light_blue('\tField text uses a common size of 50mils.')

                for field in check3_6[0]:
                    namekey = 'reference' if 'reference' in field else 'name'
                    print_white('\t\tfield: %s, text_size: %s' %
                        (field[namekey], field['text_size']))
                for text in check3_6[1]:
                    print_white('\t\ttext: %s, text_size: %s' %
                         (text['text'], text['text_size']))
                for pin in check3_6[2]:
                    print_white('\t\tpin: %s (%s), dir: %s, name_text_size: %s, num_text_size: %s' %
                        (pin['name'], pin['num'], pin['direction'], pin['name_text_size'], pin['num_text_size']))

        elif args.verbose:
            print_light_green('component: %s......OK' % component.name)
