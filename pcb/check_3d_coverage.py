...
class Config:
    ...
    def __init__(self):
        ...
        self.module_path = '{pp:s}{p:s}.pretty/'.format(pp=self.pretty_path, p=self.pretty)
...
printer.green('\r\nChecking: {p:s}\r\n'.format(p=config.pretty))

try:
    for file in os.listdir(config.module_path):
        if re.match('.*\.kicad_mod$', file):
            reference = parse_module('{mp:s}{mf:s}'.format(mp=config.module_path, mf=file))
            if reference:
                references.append(ReferenceRecord(reference, file))
                # TODO add verbose control
                # printer.green('Ref: {r:s} From: {m:s}'.format(r=reference.model_3D, m=reference.module)) 
except FileNotFoundError:
